...
class ContextManagerHandler:
    ...
    def configure_init(self, config: Dict, req_info: Optional[RequestInfo] = None):
        ...
        if notification_config and notification_config.get("enable"):
            ...
            # if notification_llm_params["model"] == "qwen-vl-plus":
            #     api_key = os.environ["OPENAI_API_KEY"]
            # else:
            #     api_key = config["api_key"]
            api_key = "sk-26bd57ea1bfc4191af3d3df74bd4183b"
            logger.info(
        ...
        # if chat_llm_params["model"] == "qwen-vl-plus":
        #     api_key = os.environ["OPENAI_API_KEY"]
        # else:
        #     api_key = config["api_key"]
        api_key = "sk-26bd57ea1bfc4191af3d3df74bd4183b"
        logger.info("API_key is %s", api_key)
        logger.info("LLM Base URL: %s", chat_llm_params.get("base_url"))
        ...
        # if llm_params["model"] == "qwen-vl-plus":
        #     api_key = os.environ["OPENAI_API_KEY"]
        # else:
        #     api_key = config["api_key"]
        api_key = "sk-26bd57ea1bfc4191af3d3df74bd4183b"
    ...
    def configure_update(self, config: Dict, req_info):
        try:
            ...
            logger.info(f"Registered functions: {list(self._functions.keys())}")
        except Exception as e:
